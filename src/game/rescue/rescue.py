...
class RescueShip:
  ...
  def random_position(self, interval: int) -> list[int]:
    ...
    y_range = random.choice(y_ranges[choice]) if isinstance(y_ranges[choice], list) else y_ranges[choice]
  ...
  def update(self, x_var: int, y_var: int, player_rect: pygame.Rect) -> None:
    ...
    if self.move and current_time - self.last_update > self.animation_delay:
      ...
      self.image = self.move_images_right[self.current_move_frame] if self.facing_right else self.move_images_left[self.current_move_frame]
  ...
  def draw_ladder(self, screen: pygame.Surface) -> None:
    ...
    ladder_x = self.rect.x + self.image.get_width() - (100 if self.facing_right else 170) * self.run.zoom * 0.5
    ...
    self.ladder_rect.update(ladder_x - 10 * self.run.zoom * 0.5, ladder_y_start + ladder_height, 30 * self.run.zoom * 0.5, 15 * self.run.zoom * 0.5)
