...
class Shooter:
    ...
    def can_shoot(self):
        ...
        return (
            self.run.mouse["press"]
            and self.run.mouse["active_click"]
            and self.run.current_shot < self.run.current_weapon_dict["charger_capacity"]
            and self.run.mouse["current_time"] - self.run.last_shot_time
            > self.run.mouse["shoot_delay"]
        )
    ...
    def handle_reload(self):
        ...
        if self.run.time >= self.run.current_weapon_dict["recharge_time"]:
            ...
            self.run.icon.resource["ammo"] -= self.run.current_weapon_dict[
                "charger_capacity"
            ]
            self.run.time = 0
        else:
    ...
    def launch_grenade(self):
        ...
        self.run.player.launch_grenade(
            direction * self.run.zoom, self.run.data_extras["grenade"]
        )
        ...
        if self.run.current_weapon_dict["delay"] == 0:
            for position in range(-30, 10, 3):
                self.run.player.launch_bullet(
                    (
                        self.run.mouse["position"][0] + position,
                        self.run.mouse["position"][1] + position,
                    ),
            ...
            for delay in range(
                0,
                self.run.current_weapon_dict["number_shoot"],
                self.run.current_weapon_dict["delay"],
            ):
                x_offset = random.randint(
                    -self.run.current_weapon_dict["precision"],
                    self.run.current_weapon_dict["precision"],
                )
                ...
                self.run.player.launch_bullet(
                    (
                        self.run.mouse["position"][0] + x_offset,
                        self.run.mouse["position"][1] + y_offset,
                    ),
                    self.run.current_weapon_dict,
                    delay,
                )
    ...
    def draw_reload_bar(self):
        ...
        pygame.draw.rect(
            self.run.screen, (100, 100, 100), (bar_x, bar_y, bar_width, bar_height)
        )
        pygame.draw.rect(
            self.run.screen, (255, 255, 255), (bar_x, bar_y, fill_width, bar_height)
        )
