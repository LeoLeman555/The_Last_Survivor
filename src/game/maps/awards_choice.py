...
class Choice:
  ...
  def choose(self, prob_A: int, prob_B: int, prob_Resource: int, prob_Food: int, prob_Weapon: int, prob_ESP: int, prob_Module: int) -> str:
    ...
    if initial_choice == 'A':
      ...
      if choice_B == 'module':
        rarity = random.choice(['Common', 'Rare'])  # Simplified choice with two options
  ...
  def weapon(self, current_weapon: int, unlock_weapons: list) -> int:
    ...
    unlocked_weapons = set(int(weapon_id) for weapon_id in unlock_weapons)  # Use set for faster look-up
    outcomes = [-1, 0, 1, 2]
