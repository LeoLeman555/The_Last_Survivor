...
class Icon:
  ...
  def update(self):
    ...
    self.resource["food"] = max(0, min(self.resource["food"], self.bars["food_max"]))
    self.resource["health"] = max(0, min(self.resource["health"], self.bars["health_max"]))
  ...
  def draw_score(self, screen: pygame.Surface, font: pygame.font.Font, value: int, x: int, y: int):
