...
class Player(pygame.sprite.Sprite):
  ...
  def update(self):
    ...
    if pygame.time.get_ticks() - self.last_regeneration_time >= 1000 and self.run.icon.resource["health"] < 100:
  ...
  def add_enemy(self, data: dict, name: str, x: int = 0, y: int = 0):
    ...
    if name.lower() in data:
      self.enemies.add(Enemy(self.run.zoom, self.run.screen, name, self, self.run.icon, x, y, data))

  def add_laser(self):
    """Add a laser attack."""
    self.lasers.add(Laser(self.run.zoom, self.enemies, self.run.data_extras["laser_probe"]))
  ...
  def add_object(self, name: str, value: int, x: int, y: int):
    ...
    self.objects.add(GroundObjects(self.run.zoom, self.run.icon, name, value, x, y, self.run.range_obj))
