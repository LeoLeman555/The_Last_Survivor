...
class Enemy(pygame.sprite.Sprite):
  def __init__(self, zoom: int, screen: 'pygame.Surface', name: str, player, icon, x: int, y: int, data: dict):
    super().__init__()
    self.data = data
  ...
  def die(self):
    ...
    if award == "weapon":
      award = self.choice.weapon(self.player.run.current_weapon_dict["id"], self.player.run.filtered_weapons.keys())
  ...
  def get_frames(self, sprite_sheet: 'pygame.Surface', row: int, num_frames: int, size_w: int, size_h: int):
