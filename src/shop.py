...
class Shop:
  def __init__(self):
    ...
    self.data_power_up_price = self.read_data.read_params("data/power_up_price.txt", "price")
    ...
    self.shop_tables_power_up = [self.create_table(dict(islice(self.data_power_up.items(), 12)), str_key=True), self.create_table(dict(islice(self.data_power_up.items(), 12, None)), str_key=True)]
    self.power_up_images = self.create_images(self.data_power_up, self.power_up_names, 3)
      ...
      for row in range(number_row):
        ...
        if draw_name != "LOCK":
          for name_text in stats.keys():
            ...
            text_rect = text.get_rect()
            text_rect.topleft = (self.back_object_rect.x, self.back_object_rect.y + stats[name_text][2])
            self.screen.blit(text, text_rect)
        ...
        if index_name != None:
          ...
          text_rect = text.get_rect()
          text_rect.topleft = (self.back_object_rect.x, self.back_object_rect.y + 180)
          self.screen.blit(text, text_rect)
          text = self.font.render(price, True, (255, 255, 255))
          text_rect = text.get_rect()
          text_rect.center = (self.back_object_rect.x + 35, self.back_object_rect.y + 210)
          self.screen.blit(text, text_rect)
  ...
  def draw_steps(self):
    ...
    if self.shop_step % 2 == 0:
      if self.button_right_arrow_rect.collidepoint(mouse_pos):
        zoomed_arrow, zoomed_rect = self.zoom_image(self.button_right_arrow, self.button_right_arrow_rect)
      ...
      if self.button_left_arrow_rect.collidepoint(mouse_pos):
        zoomed_arrow, zoomed_rect = self.zoom_image(self.button_left_arrow, self.button_left_arrow_rect)
        self.screen.blit(zoomed_arrow, zoomed_rect)
      else:
    ...
    if self.shop_step == 2:
      ...
      images = dict(islice(self.power_up_images.items(), 13)) # +1 because of the "LOCK" in the first position
      ...
      images = dict(islice(self.power_up_images.items(), 13, None)) # +1 because of the "LOCK" in the first position
      images["LOCK"] = self.power_up_images["LOCK"]
      data_names = list(self.data_power_up.keys())[12:]
      fake_names = self.power_up_names[12:]
      self.draw_elements(data, prices, stats, images, self.shop_tables_power_up[1], position, fake_names, data_names, 2, 6, 1)
  ...
  def change_level_data_power_up(self, id):
    ...
    if level <= 1:
      ...
      if self.game_data["power_up_level"][name] < 1 and self.game_data["money"] >= price:
        rewards = {"money": -price, "power_up_level": {name: 1,}}
        change_game_data = ChangeGameData(rewards)
        change_game_data.change_params(change_game_data.reward, change_game_data.game_save_data)
        self.update_data()
  ...
  def update_data(self):
    ...
    self.shop_tables_power_up = self.shop_tables_power_up = [self.create_table(dict(islice(self.data_power_up.items(), 12)), str_key=True), self.create_table(dict(islice(self.data_power_up.items(), 12, None)), str_key=True)]
    self.power_up_images = self.create_images(self.data_power_up, self.power_up_names, 3)