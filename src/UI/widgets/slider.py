...
class Slider:
    ...
    def __init__(
        ...
        self.bar_image = pygame.image.load(
            "res/options/bar_difficulty.png"
        ).convert_alpha()
        self.button_image = pygame.image.load(
            "res/options/button_difficulty.png"
        ).convert_alpha()
        self.button_click_image = pygame.image.load(
            "res/options/button_difficulty_click.png"
        ).convert_alpha()

        # Resize bar to match the given width
        self.bar_image = pygame.transform.scale(
            self.bar_image, (self.width, self.bar_image.get_height())
        )
        ...
        self.handle_x = self.x + (self.value - self.min_value) / (
    ...
    def update(self, event: pygame.event.Event) -> None:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:  # Left click
                if (
                    self.handle_x <= event.pos[0] <= self.handle_x + self.handle_width
                    and self.handle_y
                    <= event.pos[1]
                    <= self.handle_y + self.handle_height
                ):
                    self.dragging = True
            ...
            self.handle_x = max(
                ...
                min(
                    ...
                    self.x + self.width - self.handle_width,
                ),
            )
