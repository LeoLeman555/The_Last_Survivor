...
class Introduction:
  def __init__(self, screen: pygame.Surface):
    ...
    self.pause_lines = [2, 4, 9, 12]  # Indices of lines requiring a pause (1-indexed)
    self.pause_timer = 0
  ...
  def draw(self):
    ...
    if self.current_line < len(self.text_lines):  # Display current line progressively
    ...
    message = "PRESS SPACE TO CONTINUE" if self.finished_text else "PRESS SPACE TO SKIP"
      ...
      for event in pygame.event.get():
        if event.type == pygame.QUIT:
          ...
          if event.key == pygame.K_SPACE:
            if self.finished_text:  # If text is finished, close the introduction
              running = False
            else:
